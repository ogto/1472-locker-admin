...
export async function POST(req: NextRequest) {
  try {
    ...
    const upstream = new URL(
      "https://cloud.1472.ai:18443/api/v4/bread-storage/pickup-product/collect"
    );
    ...
    if (historyId) {
      upstream.searchParams.set("historyId", historyId);
    }
    ...
    return NextResponse.json(
      {
        ...
        message: response.ok
          ? "상태 변경 완료"
          : extractUpstreamMessage(parsedData) || "상태 변경 실패",
      },
      { status: response.status }
    );
  } catch (error) {
